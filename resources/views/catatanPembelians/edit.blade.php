...
@section('admin-content')
    <div class="container-fluid">
        <h1 class="h3 mb-4 text-gray-800">Edit Keterangan (Catatan) Purchase Order</h1>
                ...
                <form action="{{ route('catatanPembelians.update', $catatanPembelian->id) }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select name="project_pembelian_id" id="project_pembelian_id" class="form-control"
                            style="background-color: #f5f5f5; pointer-events: none;" onchange="this.blur();">
                            @foreach ($projectPembelians as $project)
                                <option value="{{ $project->id }}"
                                    {{ $catatanPembelian->project_pembelian_id == $project->id ? 'selected' : '' }}>
                                    {{ $project->project }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="waktu_pengiriman" id="waktu_pengiriman" class="form-control"
                            value="{{ old('waktu_pengiriman', $catatanPembelian->waktu_pengiriman) }}">
                    </div>

                    <div class="form-group">
                        <label for="alamat_pengiriman">Alamat Pengiriman</label>
                        <input type="text" name="alamat_pengiriman" id="alamat_pengiriman" class="form-control"
                            value="{{ old('alamat_pengiriman', $catatanPembelian->alamat_pengiriman) }}">
                    </div>

                    <div class="form-group">
                        <label for="contact_person">Contact Person</label>
                        <input type="text" name="contact_person" id="contact_person" class="form-control"
                            value="{{ old('contact_person', $catatanPembelian->contact_person) }}">
                    </div>

                    <div class="form-group">
                        <label for="pembayaran">Pembayaran</label>
                        <input type="text" name="pembayaran" id="pembayaran" class="form-control"
                            value="{{ old('pembayaran', $catatanPembelian->pembayaran) }}">
                    </div>
