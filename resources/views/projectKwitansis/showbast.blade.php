...
<body>
    <div class="container">
        <div class="invoice-header row" style="position: relative;">
            <div class="col-12">
                <div id="kop-brand">
                    <img src="{{ asset('kop-surat/kop-bast.png') }}" alt="Brand Image"
                        ...
                        @foreach ($projectKwitansi->pekerjaanKwitansi as $pekerjaan)
                            @foreach ($pekerjaan->batchPekerjaanKwitansi as $batchPekerjaan)
                                <tr>
                                    ...
                                    <td>{{ $batchPekerjaan->batchKwitansi->nama_batch }} <br>
                                        @php
                                            $total_uraian = 1;
                                        @endphp
                                    ...
                                    <td>{{ $batchPekerjaan->batchKwitansi->jumlah_batch }} <br>
                                        @foreach ($batchPekerjaan->batchKwitansi->uraianKwitansis as $uraian)
                                            {{ $uraian->jumlah_uraian }}
                                            <br>
                                        @endforeach
                                    </td>
                                    <td>{{ $batchPekerjaan->batchKwitansi->satuan_batch }} <br>
                                        @foreach ($batchPekerjaan->batchKwitansi->uraianKwitansis as $uraian)
                                            {{ $uraian->satuan_uraian }}
                                            <br>
                                        @endforeach
                                    </td>
                                    <td>{{ $batchPekerjaan->batchKwitansi->keterangan_batch }} <br>
                                        @foreach ($batchPekerjaan->batchKwitansi->uraianKwitansis as $uraian)
                                            {{ $uraian->keterangan_uraian }}
                                            <br>
                                        @endforeach
                                    </td>
                                </tr>
