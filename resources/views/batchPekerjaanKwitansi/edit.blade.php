        ...
        <div class="mb-3">
            ...
            <select name="pekerjaan_kwitansi_id" id="pekerjaan_kwitansi_id" class="form-select" required>
                ...
                @foreach ($pekerjaanKwitansis as $pekerjaan)
                    <option value="{{ $pekerjaan->id }}" {{ $batchPekerjaanKwitansi->pekerjaan_kwitansi_id == $pekerjaan->id ? 'selected' : '' }}>
                        {{ $pekerjaan->pekerjaan }}
                    </option>
                @endforeach
            </select>
            ...
            <div id="batchList">
                @foreach ($batchKwitansis as $batch)
                    <div class="batch-container">
                        ...
                        <ul class="list-unstyled" style="display: {{ in_array($batch->id, $selectedBatchKwitansiIds) ? 'block' : 'none' }};" id="uraian_{{ $batch->id }}">
